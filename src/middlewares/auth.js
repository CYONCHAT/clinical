...
const authenticate = (req, _res, next) => {
  try {
    ...
    if (headerTenantId && claims.tenantId && headerTenantId !== claims.tenantId) throw new AuthorizationError('Contexto de tenant inconsistente', 'TENANT_CONTEXT_MISMATCH');
    const tenantId = claims.tenantId || headerTenantId;
    if (!tenantId && !claims.service) throw new AuthorizationError('Tenant ausente no contexto', 'TENANT_CONTEXT_MISSING');
    req.auth = claims;
    req.context = {
