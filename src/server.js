...
const shutdown = async (signal) => {
  ...
  shuttingDown = true;
  logger.info({ signal }, 'clinical shutdown started');
  server.close(async () => {
...
const start = async () => {
  ...
  server.listen(env.port, env.host, () => logger.info({ service: env.serviceName, host: env.host, port: env.port }, 'clinical started'));
};
...
if (require.main === module) start().catch((error) => { logger.fatal({ err: error }, 'clinical startup failed'); process.exit(1); });
